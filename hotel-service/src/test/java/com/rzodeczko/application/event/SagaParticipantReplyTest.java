...
class SagaParticipantReplyTest {
    ...
    void fromSuccess() {
        ...
        var reply = SagaParticipantReply.from(sagaId, "HOTEL", SagaAction.RESERVE, CommandResult.success());
    ...
    void fromFailure() {
        var sagaId = UUID.randomUUID();
        var reply = SagaParticipantReply.from(sagaId, "HOTEL", SagaAction.RESERVE, CommandResult.failure("no cabins"));
